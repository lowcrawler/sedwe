$(document).on('ready',function(){
	...
	$('#takePicture').change(function () {
		...
		if(($(this).val() != '')){//Brakes the on change event loop
			//Validate if its a picture or image.
			if(valid_format_check($(this).val())){
	...
	$('#uploadImgBtn').on('click',function(){
		...
		$.ajax({
			...
			data:{dataURL:imgBase64.replace(/data:image[^;]+;base64,/,''),comment:$('#imgComment').val(),email:'fgranado-santos@usgs.gov'},
...
function valid_format_check(fileName){
		
	var allowed_extensions = new Array("jpg","png","gif","jpeg","JPG","PNG","GIF","JPEG");
...
function readURL(input) {
	if (input.files && input.files[0]) { //if file exist
		...
		reader.onload = function (e) { //when the reader loads the chosen file...
			...
			preview_image(e.target.result, function (dataURL) { //..then the is converted 
...
function preview_image(url, callback, outputFormat) {
	console.log('preview');
	...
	img.onload = function () {
		...
		imgBase64 = url;
		$('#show-picture, #imgPreview').attr('src', url); //Change source of the #show-picture image tag to the base64 string to show preview to user 
		//console.log('8: '+ new Date().getTime());
		//console.log(dataURL);
		
		callback.call(this, url); //Callback function @param {dataURL} 
		// Clean up
